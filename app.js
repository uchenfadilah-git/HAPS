...
function latestCapitalBalance(name) { return capitalRows.filter(r => r.partner === name).at(-1)?.balance || 0; }
function renderShares(totalProfit, costs) {
  els.shareBase.textContent = `Dari profit ${money(totalProfit)}`;
  els.shareCards.innerHTML = PROFIT_SPLIT.map(s=>{ const amount=totalProfit*s.percent/100; return `<article class="share-card"><div><span>${s.name}</span><strong>${money(amount)}</strong><small>${s.percent}% / ${dollar(amount/USD_TO_IDR)}</small></div><em>${s.percent}%</em></article>`; }).join("");
  const totals = costs.reduce((a,r)=>{ a.yusuf+=r.yusuf; a.shafi+=r.shafi; a.husein+=r.husein; a.total+=r.total; return a; }, { yusuf:0, shafi:0, husein:0, total:0 });
  els.costBase.textContent = `${costs.length} transaksi / ${money(totals.total)}`;
  els.costCards.innerHTML = PARTNERS.map(name=>`<article class="share-card"><div><span>${name}</span><strong>${money(totals[name.toLowerCase()])}</strong><small>Beban cost</small></div><em>Cost</em></article>`).join("");
  els.partnerPositionCards.innerHTML = PARTNERS.map(name => {
    const key = name.toLowerCase();
    const capital = latestCapitalBalance(name);
    const profitRight = totalProfit * 0.3;
    const costShare = totals[key];
    const estimatedPosition = capital - costShare + profitRight;
    return `<article class="share-card"><div><span>${name}</span><strong>${money(estimatedPosition)}</strong><small>Modal ${money(capital)} - Cost ${money(costShare)} + Hak Profit ${money(profitRight)}</small></div><em>Est</em></article>`;
  }).join("");
  els.costTable.innerHTML = costs.map(r=>`<tr><td data-label="Code">${r.code}</td><td data-label="Tracking">${r.tracking}</td><td data-label="Buying">${moneyPair(r.buying)}</td><td data-label="Delivery">${moneyPair(r.delivery)}</td><td data-label="Yusuf">${money(r.yusuf)}</td><td data-label="Shafi">${money(r.shafi)}</td><td data-label="Husein">${money(r.husein)}</td><td data-label="Source">${r.source}</td></tr>`).join("") || '<tr><td colspan="8">Data belum ada.</td></tr>';
}
