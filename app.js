...
const els = {
  ...
  partnerCards: document.querySelector("#partnerCards"), partnerTable: document.querySelector("#partnerTable"), shareBase: document.querySelector("#shareBase"), shareCards: document.querySelector("#shareCards"), costBase: document.querySelector("#costBase"), costCards: document.querySelector("#costCards"), partnerPositionCards: document.querySelector("#partnerPositionCards"), costTable: document.querySelector("#costTable"),
...
function buildAudit() {
  ...
  [...salesCodes].filter(code => !profitCodes.has(code)).forEach(code => auditItems.push({ category: "Profit belum ada", level: "danger", title: `PROFIT belum ada: ${code}`, detail: "Split profit belum tercatat di tab PROFIT." }));
  salesRows.filter(r => r.status !== "Completed").forEach(r => auditItems.push({ category: "Transaksi pending/cancel", level: "warn", title: `${r.status}: ${r.code}`, detail: `${r.item || "-"} / ${r.tracking || "tracking kosong"}` }));
  settlementRows.filter(r => r.status === "Pending" && (r.salesFeeUsd || r.buyer || r.item)).forEach(r => auditItems.push({ category: "Settlement pending", level: "warn", title: `Settlement pending: ${r.buyer || r.item || "baris eBay"}`, detail: "Actual Payoneer belum tercatat." }));
  [...salesCodes].filter(code => !costCodes.has(code)).forEach(code => auditItems.push({ category: "Cost fallback", level: fallbackCodes.has(code) ? "warn" : "danger", title: `COST belum lengkap: ${code}`, detail: fallbackCodes.has(code) ? "Dashboard pakai fallback dari REKAP." : "Tidak ada cost dan tidak bisa fallback." }));
}
function auditGroups() {
  return ["Profit belum ada", "Transaksi pending/cancel", "Settlement pending", "Cost fallback"].map(category => {
    const items = auditItems.filter(item => item.category === category);
    return { category, items, danger: items.filter(item => item.level === "danger").length, warn: items.filter(item => item.level !== "danger").length };
  });
}
function rowLimit(data) { return window.innerWidth <= 760 ? data.slice(0, 20) : data; }
function limitNote(total) { return window.innerWidth <= 760 && total > 20 ? `<p class="table-note">Mobile menampilkan 20 data pertama. Pakai filter/cari untuk mempersempit.</p>` : ""; }
function renderTableNote(tbody, total) {
  const wrap = tbody.closest(".table-wrap");
  const oldNote = wrap?.nextElementSibling?.classList?.contains("table-note") ? wrap.nextElementSibling : null;
  if (oldNote) oldNote.remove();
  wrap?.insertAdjacentHTML("afterend", limitNote(total));
}
...
function renderHealth(sales, costs, settlement, cash, profit, totalSales, buying, delivery) {
  const groups = auditGroups().filter(group => group.items.length);
  const actionItems = groups.slice(0, 4).map(group => `<div class="rank-item split-item ${group.danger ? "danger-item" : "warn-item"}"><strong>${group.category}</strong><span>${group.items.length} issue</span><small>${group.danger ? `${group.danger} wajib dicek` : "Warning"}</small></div>`).join("");
  els.businessHealth.innerHTML = actionItems || '<p class="subtitle">Semua data utama aman.</p>';
}
...
function renderShares(totalProfit) {
  els.shareBase.textContent = `Dari profit ${money(totalProfit)}`;
  els.shareCards.innerHTML = PROFIT_SPLIT.map(s => {
    const amount = totalProfit * s.percent / 100;
    return `<article class="share-card"><div><span>${s.name}</span><strong>${money(amount)}</strong><small>${s.percent}% / ${dollar(amount / USD_TO_IDR)}</small></div><em>${s.percent}%</em></article>`;
  }).join("");
}
function renderAudit() {
  els.auditCount.textContent = `${auditItems.length} issue`;
  const groups = auditGroups().filter(group => group.items.length);
  els.auditList.innerHTML = groups.map(group => `<section class="audit-group"><div class="audit-group-head"><strong>${group.category}</strong><span>${group.items.length} issue</span></div>${group.items.map(i=>`<div class="rank-item split-item ${i.level === "danger" ? "danger-item" : "warn-item"}"><strong>${i.title}</strong><span>${i.detail}</span></div>`).join("")}</section>`).join("") || '<p class="subtitle">Data health aman.</p>';
}
...
function showError(error) { console.error(error); els.status.textContent = "Gagal load data"; els.updated.textContent = `Cek akses Google Sheet / koneksi. Detail: ${error.message}`; }
